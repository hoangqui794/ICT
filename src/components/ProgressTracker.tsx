...
import { Award, CheckCircle2, Circle, RefreshCw } from 'lucide-react';
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="glass-card" style={{ padding: '32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <button
            ...
            className="nav-btn"
            style={{ fontSize: '0.85rem', color: '#EF4444', borderColor: 'rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.1)' }}
          >
            <RefreshCw size={14} /> {lang === 'vi' ? 'Đặt Lại Tiến Độ' : 'Reset Progress'}
          </button>
        </div>
        ...
        <div style={{ marginTop: '12px' }}>
          ...
          <div style={{ width: '100%', height: '14px', background: '#1E293B', borderRadius: '8px', overflow: 'hidden' }}>
            <div
              style={{
                width: `${percent}%`,
                height: '100%',
      ...
      {CURRICULUM_DATA.map((moduleItem) => {
        ...
        return (
          <div key={moduleItem.id} className="glass-card" style={{ padding: '24px' }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1.25rem', fontWeight: 700, marginBottom: '16px', color: '#FFF' }}>
                ...
                return (
                  ...
                  >
                    {isChecked ? (
                      ...
                      <Circle size={20} color="var(--text-dim)" style={{ flexShrink: 0, marginTop: '2px' }} />
                    )}
                    <div>
                      <div style={{ fontSize: '0.78rem', color: 'var(--primary)', fontWeight: 600, marginBottom: '2px' }}>
                        {item.topicTitle}
                      </div>
                      <div style={{ fontSize: '0.92rem', color: isChecked ? 'var(--text-dim)' : 'var(--text-main)' }}>
                        {text}
                      </div>
                    </div>
                  </div>
